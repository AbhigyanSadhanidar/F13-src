    ...
    <v-toolbar-items>
      ...
      <v-menu offset-y open-on-hover :close-on-content-click="closeOnCon" transition="slide-x-transition" v-for="(item, index) in items" :key="index">
        ...
        <template v-slot:activator="{ on }">
          ...
          <v-btn v-else v-on="off" small text :href="item.link" :target="item.target" class="white--text font-weight-bold text-capitalize body-2">{{item.name}}</v-btn>
        ...
        <v-list shaped style="background-color:rgba(29, 58, 124, 0.8);">
          <v-list-group v-for="(sub, subIndex) in item.options" :key="subIndex" link>
            <template v-slot:activator>

              <v-list-item v-if="sub.subMenu == true" :href="sub.redirect" :elevation="hover ? 16 : 2" :class="{ 'on-hover': hover }">
                <v-list-item-title v-text="sub.name" class="text-left white--text"></v-list-item-title>
              </v-list-item>

              <v-list-item v-else :href="sub.redirect" :target="sub.target" :elevation="hover ? 16 : 2" :class="{ 'on-hover': hover }">
                <v-list-item-title v-text="sub.name" class="text-left white--text"></v-list-item-title>
              </v-list-item>
